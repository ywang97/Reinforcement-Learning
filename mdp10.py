...
def value_iteration(mdp, q, eps = 0.01, max_iters = 1000):
    def expectation(d, f):
            return sum(d.prob(x) * f(x) for x in d.support())
    def v(s): return value(q,s)
        ...
        for s in mdp.states:
            for a in mdp.actions:
                new_q.set(s, a, mdp.reward_fn(s, a) + mdp.discount_factor * \
                          expectation(mdp.transition_model(s, a), v))
...
def Q_learn_batch(mdp, q, lr=.1, iters=100, eps=0.5,
    ...
    for i in range(iters):
        ...
        for e in range(n_episodes):
            _, episode = sim_episode(mdp, episode_length, explore)
